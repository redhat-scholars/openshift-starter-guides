...
In this shared environment, it's **NOT** possible to create a new project as in xref:projects#create_your_first_project[Create your First Project,window='_blank']

Please work in the `<your_username>-dev` project.

This environment has no Operators support, so please skip the xref:prerequisites.adoc#openshift_pipelines[OpenShift Pipelines,window='_blank'] part.
