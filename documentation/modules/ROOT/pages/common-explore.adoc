...
In this shared environment, it's **NOT** possible to create a new project as in xref:projects#create_your_first_project[Create your First Project,role='params-link',window='_blank']. The OpenShift Pipelines Operator is already installed cluster-wide, so you can skip the xref:nationalparks-pipeline.adoc#install_openshift_pipelines_from_operatorhub[Install OpenShift Pipelines,role='params-link',window='_blank'] section (you don't have the permissions to install Operators yourself anyway).
...
If available, you can also login to your ARO cluster using one of the {openshift-docs-url}/authentication/understanding-identity-provider.html[Identity Providers,window='_blank'] such as Azure Active Directory.
...
Starting with OpenShift {openshift-version}, the web console no longer splits into *Administrator* and *Developer* perspectives: there is a single, unified navigation on the left. The first page you land on is *Home -> Overview*, which summarizes the status of the resources you have access to. If you are also the Cluster Administrator, you will be presented with an overview of the whole cluster status:
...
OpenShift Web Console supports a web based Terminal called link:{openshift-docs-url}/web_console/odc-about-web-terminal.html[Web Terminal]. Instead of installing the `oc` CLI locally, you could just run this web based terminal containing useful tools such as:
...
If you want to have it in your current cluster, you can install it from *Ecosystem* -> *Software Catalog* following the link:{openshift-docs-url}/web_console/odc-about-web-terminal.html#odc-installing-web-terminal_odc-about-web-terminal[documentation].
...
The terminal opens in a drawer at the bottom of the console, on a *Terminal 1* tab. The first time you open it, OpenShift creates the terminal workspace for you, so it may take a few seconds before the prompt appears.

NOTE: If your cluster asks you to select a *Project* before starting the terminal, pick `%PROJECT%` and click *Start*.
