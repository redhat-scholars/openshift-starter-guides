...
OpenShift includes comprehensive features for automation in-cluster as seen in xref:nationalparks-codechanges-github.adoc[Webhooks with OpenShift] and as you will see in the next chapter xref:nationalparks-pipeline.adoc[Continuous Integration and Pipelines]. In addition to that, it provides great interoperability and integrations for external CI/CD systems such as Jenkins, GitLab, Azure DevOps and *GitHub Actions*.
  ...
  build-and-push: <4>
   ...
   # https://github.com/redhat-actions/s2i-build#readme
    - name: S2I Build <5>
      ...
      with:
        path_context: '.'
        builder_image: 'registry.access.redhat.com/ubi8/openjdk-17'
        image: ${{ env.APP_NAME }}
        tags: ${{ env.IMAGE_TAGS }}
  ...
  build-and-push: <4>
   ...
   # https://github.com/redhat-actions/s2i-build#readme
    - name: S2I Build <5>
      ...
      with:
        path_context: '.'
        builder_image: 'registry.access.redhat.com/ubi8/dotnet-80'
        image: ${{ env.APP_NAME }}
        tags: ${{ env.IMAGE_TAGS }}
...
<3> Here's where you define which type of trigger you want for this workflow. In this case, any change to the repository (Push) to the default branch of your fork -- the one listed under `branches` -- will trigger the action start. Check out the documentation for a full list of triggers that can be used: https://docs.github.com/en/free-pro-team@latest/actions/reference/events-that-trigger-workflows
...
NOTE: If you are using https://developers.redhat.com/developer-sandbox[Developer Sandbox for Red Hat OpenShift,window='_blank'] you can keep going: the OpenShift Pipelines Operator is now pre-installed cluster-wide on the Sandbox, so the following chapters work there too. You just need to skip the Operator installation step, since you don't have the permissions to install Operators yourself.
