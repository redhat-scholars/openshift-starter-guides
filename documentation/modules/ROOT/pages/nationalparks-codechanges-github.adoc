...
As we discussed in the previous module, OpenShift provides the Source-2-Image mechanism to create containers by defining a `BuildConfig`. In addition to that, it provides link:{openshift-docs-url}/cicd/builds/triggering-builds-build-hooks.html[triggers,window='_blank'] to control the circumstances in which the BuildConfig should be run, such as Webhooks.
...
In this lab you can use a build webhook to trigger a build execution every time there is a change in your copy of the nationalparks GitHub repository. In the OpenShift web console, navigate to your **%PROJECT%** project, then from the left navigation click **Builds -> BuildConfigs**. Click the `nationalparks` BuildConfig and scroll down to the **Webhooks** section.
...
+
--
Go to link:https://github.com/openshift-roadshow/nationalparks-quarkus[nationalparks-quarkus,window='_blank'] repository. From top-right side, click to *Fork* to start forking it into your account.
...
Then click on the `BackendController.java` file, which looks like this:

image::nationalparks-codechanges-github-change-code.png[Backend controller]

Click the edit button -- the pencil icon in the toolbar just above the code,
next to *Raw* -- then change line number 23:
...
triggered in OpenShift. From OpenShift Web Console, click **Builds -> BuildConfigs**, then **nationalparks**, and open the **Builds** tab. You should see a new one running:
