...
+
--
From the left navigation bar on the **Developer** perspective, click on **+Add**. Then choose the tile for adding **Container images** which will open up the *Deploy Image* dialog. Fill in the following details:
...
Click on **Resource type** to show its settings and make sure the **Deployment** option is chosen.
...
Click on **Deployment** to show its settings and add the following environment variables: 
...
image::mongodb_deploy_image_deployment_env_vars.png[MongoDB Image Deployment Environment Variable Settings]
...
In the **Edit Deployment** dialog, scroll down to the **Environment Variables** section and add the following four entries:
...
image::nationalparks_deployment_env_vars.png[Nationalparks Deployment Env Vars]
