...
+
--
Using the MongoDB Atlas Operator provides native integration between OpenShift and https://www.mongodb.com/atlas/database[MongoDB Atlas].
Let's take a look at how to use the Atlas Operator, which allows you the full benefits of MongoDB Atlas which still managing everything from your cluster.

.Installing the MongoDB Atlas Operator
****
TIP: The Atlas Operator is already installed on https://developers.redhat.com/developer-sandbox[Developer Sandbox] clusters

To install the MongoDB Atlas Operator, head to the Administrator Perspective and select the **OperatorHub** within the **Operators** tab.

image::mongodb-operator-selection.png[MongoDB Operator Selection]

Select the **Certified** Operator, and begin the installation process. You can leave all settings default, and click *Install* to finish the process.

image::mongodb-operator-install.png[MongoDB Operator Install]

After a few minutes, the MongoDB Atlas Operator will be installed on your cluster in the `openshift-operators` namespace and created several new custom resources to create and manage Atlas projects.
****
...
Here, copy the Organization ID, and we'll use the OpenShift Web Terminal or CLI to save this to our cluster in preparation for future use, like so:
...
image::web-terminal-org-id.png[Entering MongoDB Org ID into Web Terminal]
...
Here, save your public and private keys to your cluster enviroment, as we'll be using them in addition to the organization ID to create our Kubernetes secrets.

image::mongodb-atlas-api-key-details.png[MongoDB Atlas API Key Details]

You can do so similar to before, using the `export` command.
...
image::web-terminal-api-key.png[Entering MongoDB API Key into Web Terminal]

Now, let's specify these values to the MongoDB Atlas Operator, and create a secret. 

TIP: If you aren't using the OpenShift Developer Sandbox, replace your current namespace below with `openshift-operators`.
...
image::web-terminal-api-key-terminal.png[Creating Kubernetes secret]

You'll also need to label this secret so the Atlas Operator can find it.

[.console-input]
[source,bash,subs="+attributes,macros+"]
----
oc label secret mongodb-atlas-operator-api-key atlas.mongodb.com/type=credentials -n %PROJECT%
----

With the OpenShift secret created, we can now create an AtlasProject custom resource,
which will in turn create an Atlas Project on our MongoDB Atlas account.
