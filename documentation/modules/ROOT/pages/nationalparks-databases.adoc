...
--
////
The first step is to create Kubernetes **Secrets** which are used to store database-related configuration and credentials. From the left navigation, choose **Workloads -> Secrets**, then click the **Create** button in the upper right and select **Key/value secret**.
...
With the secret in place, click the **Quick create** (**+**) button in the top-right masthead and choose **Container images**, which will open up the *Deploy Image* form. Fill in the following details:

* Image section:
    - Image name from external registry -> `quay.io/mongodb/mongodb-community-server:8.0.32-ubi9-slim`
    - Runtime icon -> choose `mongodb`

image::mongodb_deploy_image_image.png[MongoDB Image Deployment Image Setting]

NOTE: This is MongoDB's own Community Server image, built on link:https://catalog.redhat.com/software/base-images[Red Hat UBI] and published on Quay. Earlier versions of this lab used `mongo:6.0.4` from Docker Hub; Quay has no anonymous pull rate limit, which matters when a whole room deploys at the same time.
...
IMPORTANT: If OpenShift Serverless is installed on the cluster, *Resource type* defaults to *Serverless Deployment*. Make sure you change it to *Deployment*.

* Advanced options:
    - Create a route to the application -> *uncheck* because no route is needed to the DB instance itself

image::mongodb_deploy_image_advanced.png[MongoDB Image Deployment Advanced Settings]

Still under *Advanced options*, click **Show advanced Deployment option**. Here, we'll add the following two environment variables which will be sourced from the previously created secret named `mongodb-credentials`.
...
The next step is to create a specific user in this MongoDB instance which the nationalparks application will use to create a connection. For this, go to **Workloads -> Topology**, click the **mongodb deployment**, then in the side panel open the **Resources** tab and click on the corresponding **Pod**.
...
With that, everything regarding the database is prepared and we can switch our focus back to the nationalparks application. Go back to **Workloads -> Topology**, click the **nationalparks deployment** and choose _Actions > Edit Deployment_ from the drop-down in the right view pane.

image::nationalparks_deployment_edit.png[Edit Nationalparks Deployment]

The **Edit Deployment** page opens in *Form view*. Scroll down to the **Environment Variables** section and add the entries below.

IMPORTANT: The variable names are *not* the same for every backend — each application reads its own set. Pick the tab matching the language you deployed in the previous module.
...
+
--
Set the server host and the database using *Add value*:
...
+
--
The Python application takes the database host from `DB_HOST`, and the user name from `MONGODB_USERNAME` (not `MONGODB_USER`). Using *Add value*, add:

    - `DB_HOST`: `mongodb`
    - `MONGODB_DATABASE`: `parksapp`

Then click *Add from ConfigMap or Secret* and enter `MONGODB_USERNAME` as the name. For the value, choose `mongodb-credentials` as resource and `app-usr` as key.

Again, click *Add from ConfigMap or Secret* and enter `MONGODB_PASSWORD` as the name. For the value, choose `mongodb-credentials` as resource and `app-pwd` as key.
--
====

Finally hit the *Save* button at the bottom of the form. This will trigger a re-creation of a new pod and restart the application which should now be able to successfully talk to the MongoDB instance you just deployed earlier.

[#adding_labels]
== Exercise: Adding Labels

Next, let's fix the labels assigned to the MongoDB deployment. From **Workloads -> Topology**, select the `mongodb` deployment and choose _Actions > Edit labels_.
...
mongodb-6d77f5dd8f         1         1         1       9m12s
nationalparks-5745847899   1         1         1       113s
nationalparks-865c54b745   0         0         0       21m
nationalparks-867b7d84f4   0         0         0       20m
parksmap-5b66975f9b        1         1         1       51m
----
...
[source,role="copypaste"]
----
http://www.github.com/openshift-roadshow/nationalparks-quarkus/blob/main/src/main/java/com/openshift/evg/roadshow/parks/db/MongoDBConnection.java#L51-L57
----
...
Requested Host:         nationalparks-%PROJECT%.%CLUSTER_SUBDOMAIN%
                           exposed on router default 20 minutes ago
Path:                   <none>
