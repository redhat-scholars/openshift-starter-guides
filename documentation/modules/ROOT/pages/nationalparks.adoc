...
+
--
Similar to how we used *Quick create* before with an existing image, we
can do the same for specifying a source code repository. Since for this lab you
have your own git repository, let's use it with a simple link:https://github.com/openshift-roadshow/nationalparks-quarkus.git[Java] S2I image.

From the masthead in the top-right corner of the web console, click the *Quick create* (*+*) button and choose *Import from Git*.
...
----
https://github.com/openshift-roadshow/nationalparks-quarkus.git
----
...
Select *Builder Image* as we are going to create the container image from the source code, as discussed in the next section.
...
Verify that *Java* has been selected as your Builder Image, and be sure to select *Builder Image version* *openjdk-17-ubi8* to have OpenJDK 17.

NOTE: The Java backend used by this lab lives in link:https://github.com/openshift-roadshow/nationalparks-quarkus[nationalparks-quarkus] and is built with link:https://quarkus.io[Quarkus], which requires Java 17 or newer, so the `openjdk-11-ubi8` tag used by previous versions of this lab will not work. The original link:https://github.com/openshift-roadshow/nationalparks[nationalparks] repository stays on Spring Boot for the workshops that still build it.
--
.NET::
+
--
...
Select *.NET* as your Builder Image, and be sure to select *Builder Image version* *8.0-ubi8* to run .NET 8 applications on a UBI 8 image.
--
Javascript::
+
--
...
Select *Node.js* as your Builder Image, and be sure to select *Builder Image version* *22-ubi9* for this lab.
--
Python::
+
--
...
Select *Python* as your Builder Image, and be sure to select *Builder Image version* *3.12-ubi9* for this lab.
--
====
...
In the *Build* section, leave *Build option* set to *BuildConfig*: this is the classic OpenShift build mechanism that runs the S2I process we just described.

In the *Deploy* section, set *Resource type* to *Deployment*.

[IMPORTANT]
====
When OpenShift Serverless is installed on the cluster, *Resource type* defaults to *Serverless Deployment*. Make sure you change it to *Deployment*, otherwise a Knative Service is created instead of the *Deployment*, *ReplicaSet* and *Route* objects this workshop works with.
====
...
Scroll down to the bottom and click *Labels* to expand the section, then add 3 labels.
...
The first two screenshots below show the builder image you picked, so open the tab for the language you are deploying.
...
+
--
image::nationalparks-configure-service1-java.png[Select Builder]

image::nationalparks-configure-service2-java.png[Advanced Options]
--
.NET::
+
--
image::nationalparks-configure-service1-dotnet.png[Select Builder]

image::nationalparks-configure-service2-dotnet.png[Advanced Options]
--
Javascript::
+
--
image::nationalparks-configure-service1-js.png[Select Builder]

image::nationalparks-configure-service2-js.png[Advanced Options]
--
Python::
+
--
image::nationalparks-configure-service1-py.png[Select Builder]

image::nationalparks-configure-service2-py.png[Advanced Options]
--
====

The routing and labels sections are the same for every backend:
...
To see the build logs, go to *Workloads -> Topology*, click the `nationalparks` entry, then click on *View logs* in the *Builds* section of the *Resources* tab of the side panel.
...
nationalparks-1   Source   Git@7e863e5   Running   About a minute ago   1m3s
----
...
nationalparks-1-build            0/1     Completed   0          86s
nationalparks-867b7d84f4-z5vhs   1/1     Running     0          25s
parksmap-5b66975f9b-zsktf        1/1     Running     0          31m
----
...
nationalparks   nationalparks-%PROJECT%.%CLUSTER_SUBDOMAIN%          nationalparks   8080-tcp   edge          None
