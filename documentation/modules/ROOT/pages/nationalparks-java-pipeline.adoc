...
NOTE: Ensure you met all xref:prerequisites.adoc[Prerequisites] before proceeding. xref:prerequisites.adoc#openshift_pipelines[OpenShift Pipelines] installed in your cluster and a xref:prerequisites.adoc#github_account[GitHub account] are required.
...
----
oc create -f https://raw.githubusercontent.com/openshift-roadshow/nationalparks/master/pipeline/nationalparks-pipeline.yaml -n {project-namespace}
----
...
[source,bash,subs="+attributes,macros+"]
----
oc get pipeline nationalparks-pipeline -o yaml -n {project-namespace}
----
