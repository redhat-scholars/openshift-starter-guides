...
NOTE: We will discuss in details about *DeploymentConfig* and *Deployment* in next chapters.
...
TIP: *(Optional)* Since this is a Spring app, you can mark this app as Spring application into Topology view with this additional label, it will be represented with a Spring icon:
...
NOTE: You'll notice two *Pods* here, one whose name ends with *-deploy*, and the other one with a random string as the final part of its name. The former it's a temporary Pod used by OpenShift just to deploy the latter, which is the Pod containing your application. 
