...
NOTE: Starting with OpenShift {openshift-version}, the web console no longer has separate *Developer* and *Administrator* perspectives. There is a single, unified navigation, so you no longer need to switch perspectives before starting an exercise.

Make sure your project is selected in the *Project* dropdown at the top of the page.

In the masthead (the top bar), click the *Quick create* button — the *+* icon — and choose *Container images*. This opens the *Deploy Image* form, where you can specify the information for the image you want to deploy.

image::parksmap-devconsole-container-image.png[Add from Container Image]

TIP: The *Quick create* menu is the fastest way to reach the three most common workflows: *Import YAML*, *Import from Git* and *Container images*. A larger set of options is available from *Ecosystem -> Software Catalog* in the left navigation.
...
Next to the image field you can select a *Runtime icon*, which is the icon used for the app in the Topology view. You can leave the default OpenShift icon, or since this Frontend is made with Spring Boot, you can pick *spring-boot*.

Scroll down to the *General* section and make sure to have the correct values in:
...
image::parksmap-image-options.png[Deploy image general options]

In the *Deploy* section, set *Resource type* to *Deployment*.

[IMPORTANT]
====
If OpenShift Serverless is installed on your cluster, *Resource type* defaults to *Serverless Deployment*. That is not what we want for this workshop — a Serverless Deployment scales to zero when idle and creates Knative resources instead of a plain *Deployment*. Make sure you explicitly select *Deployment* from the dropdown.
====

NOTE: We will discuss *Deployment* and *DeploymentConfig* in detail in the next chapters. *DeploymentConfig* is deprecated in recent OpenShift releases; new applications should use *Deployment*.

Scroll down to *Advanced options* and *un-check* the checkbox next to *Create a route*. For learning purposes, we will create a *Route* for the application later in the workshop.

Just below, in the *Labels* section, add some labels to better identify this deployment later. Labels will help us identify and filter components in the web console and in the command line.
...
image::parksmap-advanced-image-options.png[Deploy image advanced options]

Next, click the blue *Create* button. You will be directed to the *Topology* page, where you should see the visualization for the `parksmap` *Deployment* in the `workshop` application.

image::parksmap-dc-topology.png[Topology View with Parksmap]

TIP: You can return to this page at any time from *Workloads -> Topology* in the left navigation.
...
If you click on the `parksmap` entry in the Topology view, a side panel opens with information about that *Deployment*. The *Resources* tab may be displayed by default. If so, click on the *Details* tab.

image::switchtoresources.png[Details Tab image]

On the *Resources* tab, you will see that there is a single *Pod* that was created by your actions.
...
You can also get a list of all the *Pods* created within your *Project*, by navigating to *Workloads -> Pods* in the left navigation of the web console.
...
parksmap-5d5f84ffcc-fxmk9   1/1     Running     0          20s
----
...
[source,bash,subs="+attributes,macros+"]
----
oc get pod parksmap-5d5f84ffcc-fxmk9 -o yaml
----
...
metadata:
  ...
  creationTimestamp: "2026-09-11T08:52:57Z"
  generateName: parksmap-5d5f84ffcc-
  labels:
    app: workshop
    component: parksmap
    deployment: parksmap
    pod-template-hash: 5d5f84ffcc
    role: frontend
...............
----
...
parksmap   ClusterIP   172.30.208.102   <none>        8080/TCP   3h
----

In the above output, we can see that we have a *Service* named `parksmap` with an
IP/Port combination of 172.30.208.102/8080TCP. Your IP address may be different, as
...
From the *Topology* view, service information is available by clicking the `parksmap` *Deployment*, then *Resources*, and then you should see the `parksmap` entry in the *Services* section.
...
metadata:
  ...
  name: parksmap
  namespace: workshop
  resourceVersion: "8666578095"
  uid: 7931c747-d2c6-4103-b7a6-d21cbc65ebd0
spec:
  clusterIP: 172.30.208.102
  clusterIPs:
  - 172.30.208.102
...
----
NAME                        READY   STATUS    RESTARTS   AGE
parksmap-5d5f84ffcc-fxmk9   1/1     Running   0          5m12s
----
...
* The *Service* has a `selector` stanza that refers to `deployment=parksmap`.
...
NOTE: In older OpenShift releases these resources were created by a *DeploymentConfig*, and the label was `deploymentconfig=parksmap`. With a *Deployment*, the label generated by OpenShift is `deployment=parksmap`.
...
Selector:                 app=workshop,component=parksmap,deployment=parksmap,role=frontend
Type:                     ClusterIP
