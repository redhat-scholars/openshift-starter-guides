...
----
oc create -f https://raw.githubusercontent.com/openshift-roadshow/mlbparks/master/ose3/application-template-eap.json -n %PROJECT%
----
...
In addition to being able to instantiate templates from the command line as we did above, templates can also be instantiated from the web console. From the left navigation, click *Ecosystem* -> *Software Catalog*, filter by the *Templates* catalog type and search for `mlb`. You should see a result for `MLBparks`.

image::mlbparks-templates-dev-catalog.png[Template in the Software Catalog]
...
But as always, you can use the OpenShift web console to do the same. From the left navigation, click *Home* -> *Search*, then select *Template* from the *Resources* dropdown, and click *mlbparks*.
