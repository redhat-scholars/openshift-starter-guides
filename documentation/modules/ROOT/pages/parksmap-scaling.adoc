...
NOTE: *DeploymentConfig* and its *ReplicationController* are deprecated. In OpenShift {openshift-version} your applications use *Deployment* and *ReplicaSet*, which is what the rest of this lab shows.
...
You can also scale up to two pods from the web console. Go to *Workloads -> Topology*, click the `parksmap` *Deployment* and select the *Details* tab in the side panel:
...
parksmap-5d5f84ffcc-d9jmh   1/1     Running   0          92s
parksmap-5d5f84ffcc-fxmk9   1/1     Running   0          24m
----
...
TargetPort:               8080/TCP
Endpoints:                10.128.3.49:8080,10.128.6.215:8080
...
----
NAME       ENDPOINTS                            AGE
parksmap   10.128.3.49:8080,10.128.6.215:8080   45m
----

NOTE: On recent OpenShift releases this command also prints `Warning: v1 Endpoints is deprecated in v1.33+; use discovery.k8s.io/v1 EndpointSlice`. The `Endpoints` resource still works, but *EndpointSlice* is now the recommended API. You can get the same information with `oc get endpointslices -l kubernetes.io/service-name=parksmap`.

Your IP addresses will likely be different, as each pod receives a unique IP
within the OpenShift environment. The endpoint list is a quick way to see how
many pods are behind a service.

You can also see that both *Pods* are running in the *Topology* view:
...
[source,bash,subs="+attributes,macros+"]
----
oc delete pod parksmap-5d5f84ffcc-fxmk9 && oc get pods
----

[.console-output]
[source,bash]
----
pod "parksmap-5d5f84ffcc-fxmk9" deleted
NAME                        READY   STATUS    RESTARTS   AGE
parksmap-5d5f84ffcc-d9jmh   1/1     Running   0          4m48s
parksmap-5d5f84ffcc-r7t2x   1/1     Running   0          13s
----
