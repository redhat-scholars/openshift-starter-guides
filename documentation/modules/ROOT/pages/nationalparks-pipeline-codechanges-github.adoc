...
----
oc create -f +https://raw.githubusercontent.com/openshift-roadshow/nationalparks-quarkus/master/pipeline/nationalparks-triggers.yaml+ -n %PROJECT%
----
--
.NET::
+
...
From the left navigation, click on *Workloads* -> *Topology* to verify that a new Deployment *el-nationalparks* for the `EventListener` has been created:
...
// Go to link:https://github.com/openshift-roadshow/nationalparks-quarkus[Nationalparks,window='_blank'] repository. From top-right side, click to *Fork* to start forking it into your account.
...
+
--
From *Workloads* -> *Topology*, click the `el-nationalparks` Deployment, then in the side panel open the *Resources* tab and copy the *el-nationalparks* Route URL from the *Routes* section.
...
triggered in OpenShift. From the left navigation, click *Pipelines* -> *Pipelines*, then `nationalparks-pipeline` and open the *PipelineRuns* tab. You should see a new one running:
